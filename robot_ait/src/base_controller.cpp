...
void handle_gyro( const geometry_msgs::Vector3Stamped& imu) {
  if(count_z<100)
        count_z++;
  else if(count_z==100){count_z++;
        gyro_z_offset=imu.vector.z;}
  gyro_x = imu.vector.x;
  gyro_y = imu.vector.y;
  gyro_z = (imu.vector.z-gyro_z_offset);
...
int main(int argc, char** argv){
  ...
  ros::Subscriber gyro_sub = n.subscribe("imu", 50, handle_gyro);
  ...
  while(n.ok()){
    ...
    theta1 += dth;
    theta=alpha*theta1 + (1-alpha)*theta2*pi/180;	
